...
public class MainController {
	 ...
	 * ex) spring으로 servlet Application을 만들고 있지만 servlet 코드를 기술하지 않음
	 * Dynamic Web project : model2(mvc)를 구현시 httpServlet 상속클래스에서 Http Request 처리 --> web.xml
	 * spring에서 매핑 작업 없이 Annotation만으로 서블릿 구현
	 * 
	 * Portable (코드를 건들지 않고 다른 기술로 바꿀 수 있음)   -> 가장 중요한 의미 ㅋㅋ
	 * ex) 톰캣, 제티, 네티, 언더토우와 같은 아ㅖ 다른 기술 Stack을 소스코드 변경이 거의 없이 개발 가능
	 * 
	 * @Controller 안의 Mapping Annotation (@GetMapping, @PostMappin etc)
	...
	@RequestMapping(value = "/",method = RequestMethod.GET) //자동완성이 안되서 import 하나 추가함. RequestMethod 부분
	//@GetMapping("/")위에랑 같음. 위에서 method부분을 GET으로 해줘야 같은의미임. 안하면 GET,POST,PUSH,DELETE 다 가지고 있는 상태다.
	public ModelAndView main(Model model) {
		//model.addAttribute("title","ksmart39 mybatis"); // 이렇게하면 return "main"이고, 위에 리턴데이터 타입은 ModelAndView이 아니라 String이다. 
